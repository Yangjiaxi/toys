...
class Array
{
    ...
    */
    ...
    template <class U>
    U reduce(std::function<U(U, T)>, U);

    // .reduceRight(std::function<T(T, T)>, init_value) -> T
    // 从init_value开始对数组中元素升序执行reduce
    template <class U>
    U reduceRight(std::function<U(U, T)>, U);
...
template <class T>
Array<T>& Array<T>::push(std::initializer_list<T> il)
{
...
template <class T>
Array<T>& Array<T>::unshift(std::initializer_list<T> il)
{
...
template <class T>
std::string Array<T>::join()
{
    return join(", ");
}

template <class T>
std::string Array<T>::join(std::string spt)
{
...
template <class T>
std::ostream& Array<T>::output(std::ostream& os)
{
    std::ostringstream res;
    res << "[" << join() << "]";
    os << res.str() << std::endl;
...
template <class T>
bool Array<T>::every(std::function<bool(T)> fn)
{
    for (T& el : data)
        if (!fn(el))
            return false;
...
template <class T>
bool Array<T>::some(std::function<bool(T)> fn)
{
    for (T& el : data)
        if (fn(el))
            return true;
...
template <class T>
T* Array<T>::find(std::function<bool(T)> fn)
{
    for (T& el : data)
        if (fn(el))
            return &el;
...
template <class T>
int Array<T>::findIndex(std::function<bool(T)> fn)
{
...
template <class T>
int Array<T>::indexOf(T& t, size_t b)
{
...
int Array<T>::lastIndexOf(T&& t)
{
    return lastIndexOf(t);
}

template <class T>
int Array<T>::lastIndexOf(T& t, size_t b)
{
    for (int i = b; i >= 0; --i)
        if (data[i] == t)
            return i;
...
int Array<T>::lastIndexOf(T&& t, size_t b)
{
    return lastIndexOf(t, b);
}

template <class T>
Array<T>& Array<T>::copyWithin(size_t target)
{
    return copyWithin(target, 0, size());
}

template <class T>
Array<T>& Array<T>::copyWithin(size_t target, size_t start)
{
    return copyWithin(target, start, size());
}

template <class T>
Array<T>& Array<T>::copyWithin(size_t target, size_t start, size_t end)
{
    std::vector<T> tmp(data.begin() + start, data.begin() + end);
    for (size_t i = 0, j = target; i < tmp.size() && j < end; ++i, ++j)
        data[j] = tmp[i];
    return *this;
}

template <class T>
Array<T>& Array<T>::fill(T t)
{
    return fill(t, 0, size());
}

template <class T>
Array<T>& Array<T>::fill(T t, size_t start)
{
    return fill(t, start, size());
}

template <class T>
Array<T>& Array<T>::fill(T t, size_t start, size_t end)
{
    if (size() < end)
        data.resize(end);
...
template <class T>
Array<T>& Array<T>::sort(std::function<bool(T, T)> cmp)
{
...
template <class T>
Array<T>& Array<T>::splice(size_t start)
{
    return splice(start, size() - start, {});
}

template <class T>
Array<T>& Array<T>::splice(size_t start, size_t deleteCount)
{
                           ...
                           std::initializer_list<T> il)
{
    ...
    if (l > 0)
        data.insert(data.begin() + start, il);

    data.erase(data.begin() + start + l,
               data.begin() + start + l + deleteCount);
    return *this;
}

template <class T>
Array<T> Array<T>::from()
{
    Array<T> tmp(*this);
    return std::move(tmp);
}
...
template <class T>
Array<T> Array<T>::concat(Array<T>& B)
{
    Array tmp(*this);
...
template <class T>
Array<T> Array<T>::filter(std::function<bool(T)> fn)
{
    Array<T> tmp;
    for (auto el : data)
        if (fn(el))
...
template <class T>
template <class U>
Array<U> Array<T>::map(std::function<U(T)> fn)
{
    Array<U> tmp;
    for (auto el : data)
        tmp.push(fn(el));
...
template <class T>
Array<T> Array<T>::slice(size_t start)
{
...
template <class T>
template <class U>
U Array<T>::reduce(std::function<U(U, T)> fn, U init_value)
{
    U c = init_value;
    for (auto el : data)
        c = fn(c, el);
...
template <class T>
void Array<T>::forEach(std::function<void(T)> fn)
{
    for (auto el : data)
        fn(el);
}