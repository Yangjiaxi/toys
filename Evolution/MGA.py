...
CROSS_RATE = 0.3         # mating probability (DNA crossover)
...
class MGA(object):
    ...
    def crossover(self, loser_winner):      # crossover for loser
        ...
        loser_winner[0, cross_idx] = loser_winner[1,
                                                  cross_idx]  # assign winners genes to loser
        return loser_winner
    ...
    def mutate(self, loser_winner):         # mutation for loser
        ...
        loser_winner[0, mutation_idx] = ~loser_winner[0,
                                                      mutation_idx].astype(np.bool)
        return loser_winner
...
ga = MGA(DNA_size=DNA_SIZE, DNA_bound=[
         0, 1], cross_rate=CROSS_RATE, mutation_rate=MUTATION_RATE, pop_size=POP_SIZE)
...
for _ in tqdm(range(N_GENERATIONS)):
    ...
    if 'sca' in globals():
        sca.remove()
    sca = plt.scatter(DNA_prod, pred, s=50, lw=0, c='green', alpha=0.8)
    plt.pause(0.05)
